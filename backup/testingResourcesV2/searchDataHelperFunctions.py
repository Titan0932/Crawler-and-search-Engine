...
"""
for all the url files crawled and stored in webData directory, it creates an index map for all the urls
return a dictionary of {url: index} 
"""
def generateUrlIndexMap():
    urlIndexMap = {}
    filesList = os.listdir('webData')
    for index, file in enumerate(filesList):
        urlIndexMap[changeFilenameToLink(file)] = index
    return urlIndexMap


# """
# for all the url files crawled and stored in webData directory, it creates an index map for all the urls
# return a dictionary of {index: url} 
# """
# def generateUrlIndexMap():
#     urlIndexMap = {}
#     filesList = os.listdir('webData')
#     for index, file in enumerate(filesList):
#         urlIndexMap[index] = changeFilenameToLink(file)
#     return urlIndexMap
...
# :param urlIndexMap: A dictionary that maps a URL to an index.
# """
# def generate_probabilityTransitionMatrix(urlIndexMap):
#     totalPages = len(urlIndexMap)
#     adjacencyMatrix = []
#     for pageUrl in urlIndexMap:
#         print(pageUrl)
#         urlOutgoingLinks = get_outgoing_links(urlIndexMap[pageUrl])
#         totalOnes = len(urlOutgoingLinks)
#         newMatrix = []
#         availableones = totalOnes  # counter for ho wmany outgoing links are left
#         if totalOnes == 0:  # if no put going links then all elements=1/N
...
#                     newMatrix += [0] * (totalPages - pageIndex)
#                     break
#                 if urlIndexMap[pageIndex] in urlOutgoingLinks:
#                     newMatrix.append(1/totalOnes)
#                     availableones -= 1
#                 else:
#                     newMatrix.append(0)
