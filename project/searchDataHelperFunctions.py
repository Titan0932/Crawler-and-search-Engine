from generalHelperFunctions import changeFilenameToLink
...
def generateUrlIndexMap():
    ...
    for index, url in enumerate(dirList):
        urlIndexMap[(changeFilenameToLink(url).replace('.json', ''))] = index
    return urlIndexMap
...
def euclidean_dist(a, b):  # PS I used the same code in Tutorial 4
    ...
    for counter in range(len(a)):
        eDistance += (a[0][counter] - b[0][counter]) ** 2
